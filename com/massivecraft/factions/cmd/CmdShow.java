...
public class CmdShow extends FCommand
{
	...
	public void perform()
	{
		...
		if ( ! payForCommand(Conf.econCostShow, "aby wyswietlic informacje o miescie", "do wyswietlania informacji o miescie")) return;
		...
		{
			peaceStatus = "     "+Conf.colorNeutral+"To miasto jest pokojowe";
		}
		
		msg("<a>Dolaczanie: <i>"+(faction.getOpen() ? "zaproszenia nie sa potrzebne" : "potrzebujesz zaproszenia")+peaceStatus);

		double powerBoost = faction.getPowerBoost();
		String boost = (powerBoost == 0.0) ? "" : (powerBoost > 0.0 ? " (bonus: " : " (kara: ") + powerBoost + ")";
		msg("<a>Teren / Energia / MAX energia: <i> %d/%d/%d %s", faction.getLandRounded(), faction.getPowerRounded(), faction.getPowerMaxRounded(), boost);

		if (faction.isPermanent())
		{
			msg("<a>To miasto jest wieczne, istnieje nawet bez mieszkancow.");
		}
		...
		if (Econ.shouldBeUsed())
		{
			...
			if (value > 0)
			{
				...
				msg("<a>Calkowita wartosc terenu: <i>" + stringValue + stringRefund);
			}
			
			//Show bank contents
			if(Conf.bankEnabled) {
				msg("<a>W banku: <i>"+Econ.moneyString(Econ.getBalance(faction.getAccountId())));
			}
		}
		...
		// List relation
		String allyList = p.txt.parse("<a>Sojusze: ");
		String enemyList = p.txt.parse("<a>Wrogowie: ");
		...
		// List the members...
		String onlineList = p.txt.parse("<a>")+"Czlonkowie online: ";
		String offlineList = p.txt.parse("<a>")+"Czlonkowie offline: ";
