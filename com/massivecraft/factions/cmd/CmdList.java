...
public class CmdList extends FCommand
{
	...
	public void perform()
	{
		...
		if ( ! payForCommand(Conf.econCostList, "aby wyswietlic liste miast", "do wyswietlania listy miast")) return;
 ...
 */
		...
		lines.add(p.txt.titleize("Lista miast "+pagenumber+"/"+pagecount));

		for (Faction faction : factionList.subList(start, end))
		{
			if (faction.isNone())
			{
				lines.add(p.txt.parse("<i>Bezdomni<i> %d online", Factions.i.getNone().getFPlayersWhereOnline(true).size()));
				continue;
			}
			lines.add(p.txt.parse("%s<i> %d/%d online, %d/%d/%d",
				faction.getTag(fme),
