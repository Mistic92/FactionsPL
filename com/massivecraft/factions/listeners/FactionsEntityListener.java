...
public class FactionsEntityListener implements Listener
{
	...
	public void onEntityDeath(EntityDeathEvent event)
	{
		...
		if (faction.isWarZone())
		{
			...
			{
				powerLossEvent.setMessage("<i>Nie straciles zadnej energi od czasu pobytu w strefie walki.");
				powerLossEvent.setCancelled(true);
			}
			if (Conf.worldsNoPowerLoss.contains(player.getWorld().getName()))
			{
				powerLossEvent.setMessage("<b>Swiat na ktorym jestes posiada wylaczone tracenie energi jednak nadal tracisz energie od czasu pobytu w strefie walki.\n<i>Twoj poziom energi to <h>%d / %d");
			}
		}
		else if (faction.isNone() && !Conf.wildernessPowerLoss && !Conf.worldsNoWildernessProtection.contains(player.getWorld().getName()))
		{
			powerLossEvent.setMessage("<i>Nie straciles zadnej energi od czasu pobytu w dziczy.");
			powerLossEvent.setCancelled(true);
		}
		...
		{
			powerLossEvent.setMessage("<i>Nie straciles zadnej energi od czasu gdy jestes w pokojowej nacji.");
			powerLossEvent.setCancelled(true);
		}
		else {
			powerLossEvent.setMessage("<i>Twoj poziom energi to <h>%d / %d");
		}
	...
	public boolean canDamagerHurtDamagee(EntityDamageByEntityEvent sub, boolean notify)
	{
		...
		if (defLocFaction.noPvPInTerritory()) {
			if (damager instanceof Player)
			{
				if (notify)
				{
					...
					attacker.msg("<i>Nie mozesz atakowac innych graczy w "+(defLocFaction.isSafeZone() ? "bezpiecznej strefie." : "pokojowym terytorium."));
				}
				return false;
			}
		...
		{
			if (notify) attacker.msg("<i>Nie mozesz atakowac innych graczy przez " + Conf.noPVPDamageToOthersForXSecondsAfterLogin + " sekund od zalogowania sie.");
			return false;
		}
		...
		{
			if (notify) attacker.msg("<i>Nie mozesz atakowac innych graczy gdy jestes  "+(locFaction.isSafeZone() ? "w bezpiecznej strefie." : "na pokojowym terytorium."));
			return false;
		}
		...
		{
			if (notify) attacker.msg("<i>Nie mozesz atakowac innych graczy dopuki nie dolaczysz do miasta.");
			return false;
		}
		else if (defendFaction.isNone())
		{
			...
			{
				if (notify) attacker.msg("<i>Nie mozesz atakowac graczy ktorzy nie naleza do zadnego miasta.");
				return false;
			}
		}
		...
		{
			if (notify) attacker.msg("<i>Nie mozesz atakowac graczy ktorzy sa w miescie o pokojowym nastawieniu.");
			return false;
		}
		else if (attackFaction.isPeaceful())
		{
			if (notify) attacker.msg("<i>Nie mozesz atakowac graczy gdy jestes w miescie o pokojowym nastawieniu.");
			return false;
		}
		...
		{
			if (notify) attacker.msg("<i>Nie mozesz atakowac neutralnych miast. Ustaw ich jako wrogow.");
			return false;
		}
		...
		{
			if (notify) attacker.msg("<i>Nie mozesz atakowac %s<i>.", defender.describeTo(attacker));
			return false;
		}
		...
		if (ownTerritory && relation.isNeutral())
		{
			if (notify)
			{
				attacker.msg("<i>Nie mozesz zaatakowac %s<i> na ich terytorium dopuki nie oznaczysz ich jako wrogow.", defender.describeTo(attacker));
				defender.msg("%s<i> chcial cie zgwalcic.", attacker.describeTo(defender, true));
			}
			return false;
		}
		...
		if (damage > 0.0 && ownTerritory && Conf.territoryShieldFactor > 0)
		{
			...
			if (notify)
			{
				...
				defender.msg("<i>Obrazenia wroga zostaly zredukowane do <rose>%s<i>.", perc);
			}
		}
